	...
	function makeTweet(arr){
		...
		for (var i = 0; i<arr.length; i++){
			if (arr[i] != null){
				...
				tweet.score = ((arr[i]["retweet_count"]) + (arr[i]["retweet_count"]))/5; 
				tweets.push(tweet);
			}
		};
		return tweets;
...
//	}
	...
	if(sbool){
		...
		request.open("POST", "./store-s-alt.php", true);
...
//	};
	...
	toptentweets = topsorttweet.slice(10);
	buffer = topsorttweet.slice(9,10);
	for(var i=0;i<toptentweets.length;i++){
		if(toptentweets[i].score > 800){
			toptentweets[i] = buffer[0]
		};
	};
